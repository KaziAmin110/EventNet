...
const populateDatabase = async () => {
    try {
        ...
        const feedMapper = await Promise.all(allEvents.map(async (e) => {
            ...
            if (!rso_id) {
                console.log('Cannot insert the current event. Its RSO could not be found or inserted');
                return null;
            }
        ...
        console.log('Error, could not populate the database with the UCF event feed', err);
    }
};
